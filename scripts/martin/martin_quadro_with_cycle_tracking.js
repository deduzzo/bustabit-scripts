...
function onGameStarted() {
    ...
    if (normalModeProfit >= cycleTargetProfit) {
        ...
        logV(1, `CYCLE #${totalCycles} WON | Profit: +${(normalModeProfit/100).toFixed(0)} bits | WR: ${((wonCycles/(wonCycles+lostCycles))*100).toFixed(1)}% (${wonCycles}W/${lostCycles}L) | Max Loss Streak: ${maxConsecutiveLossCycles}${streakDistribution} | VB: ${(currentWorkingBalance/100).toFixed(0)} bits`);
