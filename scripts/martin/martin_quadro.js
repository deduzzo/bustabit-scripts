 ...
 *
 * Sequenza: betâ, betâ=betâ*mult, betâ=betâ*multÂ², ..., betâ=betâ*mult^(N-1)
 * Perdite totali: L = betâ * (1 + mult + multÂ² + ... + mult^(N-1))
 *                   = betâ * (mult^N - 1) / (mult - 1)
 *
 * Bet vincente: betâââ = betâ * mult^N
 * Profit: P = betâââ * (payout - 1)
 *
 * Condizione: P >= L
 * betâ * mult^N * (payout - 1) >= betâ * (mult^N - 1) / (mult - 1)
 *
 * Semplificando (betâ si cancella):
...
function calculateNormalMultiplier(payout, maxLosses) {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if (testMult(mid)) {
            // Mult troppo alto, prova piÃ¹ basso
            result = mid;
            high = mid;
        } else {
            // Mult troppo basso, prova piÃ¹ alto
            low = mid;
        }
...
function calculateRecoveryBet() {
    ...
    // Es: 100 bits perdite, payout 1.5x â bet = 100 / 0.5 = 200 bits
